<template >
  <vx-card ref="card" class="mb-base relative">
    <div class="flex justify-between flex-col-reverse sm:flex-row">
      ...
      <div class="vx-card__action-buttons flex ml-auto">
        ...
        <vs-popup class="text-center" title="Supprimer L'exercice " :active.sync="delSinglePopup">
          <p
            class="mb-4 text-center"
          >Etes vous sur de vouloir supprimer cet exercice? Tout le contenu sera perdu</p>
          <div class="flex justify-center">
            <vs-button class="mr-2" @click="delSinglePopup=false" color="warning" type="filled">Non</vs-button>
            <vs-button
              class="ml-2"
              @click="deleteblock_from_parent()"
              color="danger"
              type="filled"
            >Oui</vs-button>
          </div>
    ...
    addblock(type, pos) {
      ...
      switch (type) {
        case "question":
          ...
          //   const block = self.block.children[self.block.children.length - 1];
          //   const target = $(
        ...
        case "text":
          if (self.checkOthertext(self.block)) {
            return this.$vs.notify({
              title: "",
              text: "Cet exercice contient deja un block texte",
              color: "warning",
              iconPack: "feather",
              icon: "icon-alert-circle",
            });
